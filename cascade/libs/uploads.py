...
def save_ticket_records(line, site, file_record):
    try:
        ...
        ticket = Ticket.objects.get(site_id=site, id=system_id)
        ...
        if ticket.status.service_status != 'Completed':
            ...
            if comment and type(comment) is str:
                ticket_comment = TicketComments(site_id=site, text=comment + " -Imported from reader device: %s" % device_name, ticket=ticket, created_by=file_record.uploaded_by)
                ticket_comment.save()
            ...
            if ticket.service_type.code == 'REPAIR':
                ...
                ticket.status = TicketStatus.objects.get(site_id=site, service_status='Unsuccessful')
