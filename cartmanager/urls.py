...
from views import CartUploadView, DataErrorsView, CustomerUploadView, TicketsDownloadView, TicketsCompletedUploadView, \
      CartProfileAPI, CartSearchAPI, CustomerProfileAPI, LocationProfileAPI, UpdateCartLocationAPI, CartSearch
...
urlpatterns = patterns('cartmanager.views',
    ...
    url(r'^app/cart/profile/(?P<pk>\d+)/$', CartProfileAPI.as_view(), name='cart_api_profile'),
    url(r'^app/cart/profile/(?:/(?P<pk>\d+))?$', CartProfileAPI.as_view(), name='cart_app_profile'),
